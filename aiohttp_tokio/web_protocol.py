...
from aiohttp.helpers import CeilTimeout, create_future, ensure_future
from aiohttp.http import (HttpProcessingError, HttpRequestParser, PayloadWriter,
                          StreamWriter)
...
class RequestHandler(asyncio.streams.FlowControlMixin, asyncio.Protocol):
    ...
    def data_received(self, request):
        ...
        if self._waiters:
            ...
            handler = ensure_future(
                self.start(request, data), loop=self._loop)
            data.append(handler)
    ...
    @asyncio.coroutine
    def start(self, request, handler):
        ...
        while not self._force_close:
            ...
            request.time_service = self._time_service
            try:
                try:
                ...
                if self.transport is None:
                    ...
                    if self._messages:
                        request = self._messages.popleft()
                    else:
